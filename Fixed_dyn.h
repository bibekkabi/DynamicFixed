typedef unsigned long int FP32 ; // unsignd long int 32 bit in size
...
FP32 fconv( FP32 a , FP32 q1 , FP32 q2);// Convrsion from q1 format to q2 format
FP32 Get_Optimum_Q(float flt_a);// Get Q format depending on range
FP32 tofix(float a, FP32 q); // Floating-point to fixd-point format(q) 
float toflt(FP32 a , FP32 q); // Fixed-point (q) to floating-point format
