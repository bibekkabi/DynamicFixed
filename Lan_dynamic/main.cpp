   ...
   //  initializing r matrix to N x 1 identity vector ( eye vector)
		...
		for(i=1;i<SIZ;i++)
          { tofix(0,&r[i]);
            ...
            tofix(0,&wOld[i]);
            tofix(0,&wCur[i]);
            tofix(0,&qCur[i]);// initialize qCur
            }
...
// Orthogonality estimates
   ...
   // wOld[0]= (E[0]*wCur[1] + D[0]*wCur[0] - D[j]*wCur[0] - E[j-1]*wOld[0])/E[j] + EPS*(E[0]+E[j])*0.3*(random_normal());  
    ...
    tofix(EPS*SIZ*((toflt(fix_E[1].fix_val,fix_E[1].Q_val)/toflt(fix_E[j].fix_val,fix_E[j].Q_val)))*0.6*(random_normal()),&wCur[j] ) ;
    //wCur[j+1] = 1.0 ;
	tofix(1.0,&wCur[j+1]);
	...
	tofix(EPS*1.5*(random_normal()),&wCur[k]);
    }
    nVec = nVec + j ;
    tofix(0,&fix_E[j]);
